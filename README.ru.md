...
**Что это в 5 предложениях.** My Claude Bot превращает твою подписку Claude Pro ($20/мес) или Claude Max ($100/мес, $200/мес) во флот AI-агентов в Telegram, где `me` — мастер-оркестратор и точка входа, который настраивает всё остальное и делегирует задачи sandbox-воркерам: `coder`, `team`-хабу, `archivist`'у, плюс неограниченно кастомных агентов, которых можно поднимать на лету. Только у `me` неограниченный доступ; каждый воркер заперт в своей папке `agents/{имя}/` со своим списком разрешённых инструментов и не может ничего прочитать или записать за её пределами. Агенты общаются через MessageBus, помнят всё в wiki-памяти с git-версионированием и самообучаются по ночам через Dream-циклы, которые анализируют паттерны использования и предлагают новые скиллы или улучшения схемы. Работает в личных чатах и группах, принимает файлы и голосовые, ставит скиллы из общего маркетплейса сообщества и крутится без присмотра на любом дешёвом VPS. Никаких API-счетов, никакой оплаты за вызовы — одна команда поставить, одна команда обновить, фиксированная плата в месяц за всё.

```
                     Telegram  (личные чаты + группы + топики)
                                  │
                                  ▼
                      ┌──────────────────────┐
                      │          me          │ ◄── мастер / оркестратор
                      │  полный доступ, все  │     точка входа, настраивает
                      │  тулы, создаёт       │     и делегирует воркерам
                      │  других агентов      │
                      └──────────┬───────────┘
                                 │
                                 ▼ делегирует через MessageBus
            ┌──────────┬─────────┴─────────┬────────────┐
            │          │                   │            │
            ▼          ▼                   ▼            ▼
          coder       team             archivist     custom…
          (код)     (группа)             (докум.)   (свои)
            │          │                   │            │
            └──────────┴───────────────────┴────────────┘
             sandbox-воркеры: каждый заперт в agents/{имя}/
             со своим allowlist тулов, не трогает чужие папки
...
- **Мастер + воркеры** -- `me` это мастер-оркестратор и точка входа с полным доступом, настраивает всё остальное и делегирует задачи воркерам; `coder` (разработка), `team` (групповой хаб) и `archivist` (domain-agnostic документный архив) идут в коробке как готовые sandbox-воркеры
- **Multi-agent fleet** -- неограниченно добавляй своих агентов поверх базовых, каждый со своим Telegram-ботом, SOUL и скиллами; все не-мастер агенты наследуют sandbox
- **Делегирование агентов** -- иерархия master/worker, Orchestrator маршрутизирует сообщения через MessageBus
- **Sandbox** -- каждый не-мастер агент работает в своей папке `agents/{имя}/` со своим allowlist инструментов, не может читать или писать за её пределами; только у `me` неограниченный доступ
...
cron:
  ...
  - name: "inventory_last_day"
    schedule: "0 10 L * *"          # последний день месяца в 10:00
    prompt: "Напомни команде про инвентаризацию"
    model: "haiku"
    notify: true
    chat_id: -1001234567890         # в рабочую группу, а не в личку
...
**Последний день месяца.** В поле дня работают `L` (последний день) и `L-N` (за N дней до последнего) — число дней в месяце считается на лету, поэтому одно правило корректно и для 28/29, и для 30/31:

| Выражение | Когда |
|---|---|
| `0 10 L * *` | последний день месяца, 10:00 |
| `0 16 L-1 * *` | накануне последнего дня, 16:00 |
| `0 9 L,L-1 * *` | оба этих дня, 9:00 |

**Куда уходит отчёт.** По умолчанию — в дефолтный чат агента (личка владельца). Поля `chat_id` и `thread_id` перекрывают адрес: так задача пишет в группу или в конкретный топик форума. Те же поля работают и у триггеров `heartbeat.triggers`.

**Каким ботом.** Без `chat_id` у worker-агента (у него нет лички с владельцем) отчёт уходит через бота мастер-агента. С явным `chat_id` — через собственного бота агента: в группу добавляют именно его, мастер-бот там обычно не состоит, и отправка через него молча падает с `chat not found`. Так что бот агента должен быть участником указанной группы.

Расписание живёт в `agent.yaml` (файл на диске, под git) — задачи переживают рестарт бота и `git pull`. Это не то же самое, что ин-сессионные напоминания Claude-раннера: те живут только пока жива сессия.
...
- **Изоляция master/worker:** только у `me` (мастер-оркестратора) неограниченный доступ; каждый остальной агент работает в своей папке `agents/{имя}/` со своим allowlist инструментов и не может читать или писать за её пределами
