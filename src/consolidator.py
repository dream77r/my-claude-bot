...
Конфиг в agent.yaml:
  consolidator:
    enabled: true
    max_turns: 30          # макс. пар (вопрос-ответ) до сжатия
    max_chars: 80000       # макс. символов (вход+выход) до сжатия
...
class Consolidator:
    ...
    def __init__(self, agent_dir: str, config: dict):
        ...
        self.max_chars = config.get("max_chars", 80_000)
    ...
    def needs_consolidation(self) -> bool:
        ...
        return (
            self._turns >= self.max_turns
            or self._total_chars >= self.max_chars
        )
