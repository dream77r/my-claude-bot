...
class Agent:
    ...
    def build_group_system_prompt(self, chat_id: int) -> str:
        ...
        # Важно: НЕ пишем «отвечай только когда к тебе обращаются» — LLM
        # воспринимает буквально, не видит снятый бриджем @-тег и шлёт
        # «(нет @упоминания — молчу)» в чат. Бридж фильтрует входящие на
        # своей стороне (см. src/bridge/messages.py): если до агента дошёл
        # turn — значит его уже позвали через mention/reply/cron.
        group_instructions = (
            "## Режим группового чата\n\n"
            "Ты в групповом чате. Если этот turn к тебе пришёл — значит "
            "бридж на стороне Python уже отфильтровал входящие: тебя "
            "тегнули `@username`, ответили реплаем на твоё сообщение или "
            "сработал cron. **Отвечай нормально.** Бридж снимает `@username` "
            "из текста перед передачей — поэтому в видимом тебе тексте "
            "символа `@` уже нет; не перепроверяй упоминание и **не пиши** "
            "слова «молчу», «нет упоминания», «(не вижу @)» — это команда "
            "тебе не отвечать, а не текст в чат.\n\n"
            "Правила:\n"
