...
## МАКС messenger (optional second transport)

Любой агент может жить одновременно в Telegram и в мессенджере **МАКС** (VK) —
одна и та же память, навыки, личность; ответ уходит туда, откуда пришло
сообщение. Код один на всех; кто склонировал репозиторий — включает МАКС у себя
своим токеном.

**Включение (zero-config, без правки `agent.yaml`):**

1. Создай бота в МАКС (у `@MasterBot`) и получи токен.
2. Добавь в `.env` переменную по имени агента — `{ИМЯ}_MAX_BOT_TOKEN`:
   ```bash
   ME_MAX_BOT_TOKEN=your_max_bot_token_here
   ```
3. `./update.sh` (поставит `maxapi`) и перезапусти бота.

Нет переменной → агент работает только в Telegram, ничего не меняется.
Альтернатива — явное поле `max_bot_token: "${ME_MAX_BOT_TOKEN}"` в `agent.yaml`.

> **v1.** Текст в обе стороны, контроль доступа (`allowed_users`), логирование в
> память. Исходящие файлы пока приходят текстовой пометкой; богатый набор
> команд (`/stop`, `/restart` и т.п.) — только в Telegram. Публикация публичного
> бота в МАКС требует верификации юрлица РФ (для личного бота не критично).
